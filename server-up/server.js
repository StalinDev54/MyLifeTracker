...
function checkCriticalFiles() {
  ...
  if (!fs.existsSync(FOCUS_TASKS_FILE)) {
    ...
    fs.writeFileSync(FOCUS_TASKS_FILE, JSON.stringify(initialFocusTasks, null, 2), "utf8");
...
app.get("/api/cloudmusic", async (req, res) => {
  try {
    ...
    // 当前播放的歌曲应该是听歌记录的第一条（最新的）
    const currentPlayingSong = musicLogs.length > 0 ? musicLogs[0] : null;
...
app.post("/api/focus-tasks/toggle", async (req, res) => {
  try {
    ...
    // 查找进行中的任务
    if (action === 'pause') {
      const taskIndex = currentTasks.inProgress.findIndex(t => t.id === taskId);
      if (taskIndex !== -1) {
        taskFound = true;
        // 将任务从进行中移动到待办
...
app.get("/Getxingzhe", async (req, res) => {
  try {
    ...
    const XINGZHE_TOKEN = process.env.XINGZHE_TOKEN || "be972c4bab76fd44cee8de08f2209531e7a75f95";
...
// 查看指定ID的长途骑行详情API
app.get("/Getxingzhe/:id", async (req, res) => {
  try {
    console.log(`【接口请求】GET /Getxingzhe/${req.params.id} - 来源：${req.ip}`);
    
    // 行者运动API配置
    const XINGZHE_TOKEN = process.env.XINGZHE_TOKEN || "01bdd28fb2b24d2275f64407545bfc67bc20a502";
    const activityId = req.params.id;
    
    // 构建API请求URL
    const XINGZHE_API_URL = `https://www.imxingzhe.com/openapi/v1/activities/${activityId}/`;
    
    // 发送请求到行者运动API
    const response = await axios.get(XINGZHE_API_URL, {
      headers: {
        "Authorization": `Bearer ${XINGZHE_TOKEN}`,
        "Content-Type": "application/json",
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
    ...
    console.error(`【行者运动详情API】请求失败：${error.message}`);
    res.status(500).json({
      success: false,
      error: "行者运动详情数据获取失败",
      detail: error.response?.data || error.message,
...
app.post("/postStalin", async (req, res) => {
  try {
    ...
    // 判断是否为同一状态下的电量更新
    const isBatteryUpdate = latestSameDeviceLog && 
      latestSameDeviceLog.status === newLogEntry.status &&
...
async function ensureFocusTasksFile() {
  try {
    ...
    await fsPromises.writeFile(FOCUS_TASKS_FILE, JSON.stringify(initialFocusTasks, null, 2), "utf8");
...
async function readFocusTasks() {
  ...
  try {
    ...
    await fsPromises.writeFile(FOCUS_TASKS_FILE, JSON.stringify(initialFocusTasks, null, 2));

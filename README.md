...
- **在线体验：** [chat.jiclub.site](https://chat.jiclub.site) - 与你的数字生命对话
...
| 监控项 | 实时能力 | 数据示例 | AI 联动 |
|--------|---------|---------|---------|
| 应用使用 | 实时记录当前使用的 APP | "抖音 - 使用中 - 45分钟" | "你已经刷抖音 1 小时了" |
| 锁屏状态 | 监听锁屏/解锁事件 | "锁屏 - 23:30" | "你该睡觉了，明天还要早起" |
| 电池状态 | 实时电量、充电状态 | "电量 20% - 未充电" | "电量不足，记得充电" |
| 应用切换 | 记录应用切换频率 | "微信 → B站 → 抖音" | "你今天切换了 50 次应用" |

**配置指南：** [android-up/README.md](android-up/README.md) | [AutoTask 配置教程](android-up/autotask-guide.md)
...
基于 [小智 AI](https://xiaozhi.me) 和 [chat.jiclub.site](https://chat.jiclub.site)：
...
2. 数字生命通过语音播报："早上好，昨晚睡了 7.5 小时，今天天气晴朗，适合骑行"
...
3. 你可以通过 chat.jiclub.site 查看详细的骑行轨迹和数据统计

#### 场景 4：睡前总结

1. 你拨打数字生命的电话："今天我做了什么？"
2. 数字生命语音回复："今天你工作了 8 小时，听了 30 首歌，骑行 30km，刷了 1 小时 B 站，完成了 5 个任务，表现不错！"
3. 你锁屏睡觉（Android 自动上传锁屏事件）
4. 数字生命："晚安，明天会更好"
...
1. 关注 [作者博客](https://blog.jiclub.site) 或项目仓库，获取最新动态
2. 发送邮件至项目作者（邮箱见博客），邮件主题格式：「【开发招募】- 角色 - 姓名」
3. 邮件内容需包含：
   - 个人简介（含技术栈、过往项目/学习经历）
   - 可投入时间（如：每周 5-8 小时）
...
3. **Q：如何与数字生命对话？**  
   ...
   - 文本聊天：访问 [chat.jiclub.site](https://chat.jiclub.site) 发送消息

4. **Q：Android 手机监控会耗电吗？**  
   A：不会。使用 AutoTask 等自动化应用只在特定事件（锁屏、解锁、应用切换）时触发，不会常驻后台，耗电量极低。

5. **Q：网易云音乐监听服务会影响客户端性能吗？**  
...
10. **Q：如何在 Android 手机上上传设备使用日志？**  
    A：推荐使用 **AutoTask** 自动化应用（免费开源）配置自动上传任务。也可以使用 Tasker、MacroDroid 等应用。详细配置步骤请参考 [android-up/README.md](android-up/README.md) 和 [android-up/autotask-guide.md](android-up/autotask-guide.md) 文档。无需编写代码，通过可视化配置即可实现。如需 AutoTask 配置教程，请联系项目作者。

11. **Q：设备日志会记录哪些信息？**  
    A：仅记录设备名称、锁屏状态、电池电量、充电状态、当前应用包名。不会记录应用内容、聊天记录、密码等敏感信息。所有数据仅用于个人使用统计。详见 [android-up/README.md](android-up/README.md#数据说明)。
...
📞 **立即体验：** [chat.jiclub.site](https://chat.jiclub.site) - 与你的数字生命对话
